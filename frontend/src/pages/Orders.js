...
const Orders = () => {
  ...
  try {
    // 1. Aggiorna utente (punti, storico ordini, premi)
    const resUser = await axios.put(`http://localhost:5000/api/users/${email}/order`, {
      points: totalPoints,
      products: cart,
      appliedReward: appliedReward || null
    });

    // 2. Crea documento Order separato
    ...
    setUser(resUser.data); // aggiorna utente locale
    setCart([]);
    setAppliedReward(null);
  ...
  <div className="rewards-apply">
    <p>🎁 Premi riscattati disponibili (puoi applicarne uno per ordine):</p>
    <div className="rewards-list">
      {user.rewards
        .filter(r => {
          // filtra gli sconti già usati
          if (r.includes("Sconto")) {
            return !user.usedDiscounts?.map(d => d.trim().toLowerCase()).includes(r.trim().toLowerCase());
          }
          return true; // i gift restano
        })
        .map((r, i) => {
      ...
      <div className="orders-buttons-wrapper">
        <div className="orders-buttons-container">
          <button className="final-btn" onClick={() => navigate("/homepage")}>🏠 Torna alla Home</button>
          <button className="final-btn" onClick={() => navigate("/catalogo-prodotti")}>📦 Catalogo Prodotti</button>
          <button className="final-btn" onClick={() => navigate("/orders-list")}>📋 Lista Ordini</button>
        </div>
      </div>
    </div>
  );
