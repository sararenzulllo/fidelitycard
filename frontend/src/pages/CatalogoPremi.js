...
const CatalogoPremi = () => {
  ...
  const addPrize = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/api/prizes", {
        name: newPrizeName,
      ...
      console.error("Errore Axios:", err.response?.data?.message || err.message);
  ...
  const deletePrize = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/prizes/${id}`);
    ...
    <div className="catalogo-page">
      <div className="catalogo-container">
        ...
        <div className="orders-buttons-container">
          ...
          <button className="final-btn" onClick={() => navigate("/riscatta-premio")}>🎁 Riscatta premio</button>
        </div>
      </div>
    </div>
  );
